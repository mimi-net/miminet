...
DINAMYC_PORT_FILE_NAMES = [
    ("tcp_connection_setup_1_network.json", "tcp_connection_setup_1_answer.json", r'TCP \(SYN\) \d+', len('TCP (SYN) '), r'port'),
    ("icmp_network_unavailable_network.json", "icmp_network_unavailable_answer.json", r'TCP \(SYN\) \d+', len('TCP (SYN) '), r'port'),
    ("icmp_host_unreachable_network.json", "icmp_host_unreachable_answer.json", r'TCP \(SYN\) \d+', len('TCP (SYN) '), r'port'),
...
DINAMYC_PORT_TEST_CASES = [
    ...
    for (network, answer, pattern, length, replace) in [list(read_files(case[0], case[1])) + [case[2], case[3], case[4]] for case in DINAMYC_PORT_FILE_NAMES]
]
