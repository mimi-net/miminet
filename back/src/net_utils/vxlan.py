    ...
    for node in nodes:
        if node.config.type == "router":
            ...
            for iface in node.interface:
                ...
                target_ips = iface.vxlan_vni_to_target_ip  # Expected to be List[List[str]]
...
def setup_network_interface(router: 'Node', intf: str, local_ip: str, target_ips: List[List[str]]) -> None:
    """
    ...
    for vni in vxlan_vnis:
        ...
        # Add VXLAN interface
        router.cmd(f'ip link add {vxlan_name} type vxlan id {vni} local {local_ip} '
