...
class TestJobEdit:
    ...
    def test_host_edit_ping_job(self, authorized_page: Page):
        ...
        config1 = network.open_node_config(host1_id)
        authorized_page.click(f"#config_host_job_edit_{initial_job_id}")

        # Job field is pre-filled with original value (no fixed sleep)
        expect(
            authorized_page.locator(
                Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
            )
        ).to_have_value("192.168.1.2")
        ping_field = authorized_page.locator(
            Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
        )

        # Modify the job
        ping_field.fill("192.168.1.100")
    ...
    def test_edit_job_preserves_job_limit(self, authorized_page: Page):
        ...
        config1 = network.open_node_config(host1_id)
        authorized_page.click(f"#config_host_job_edit_{first_job_id}")
        ...
        ).to_have_value(first_job_original_ip)

        # Modify the ping target
        ping_field = authorized_page.locator(
            Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
        )
        ping_field.fill("192.168.5.99")
    ...
    def test_edit_multiple_jobs_in_sequence(self, authorized_page: Page):
        ...
        # Edit first job
        config_host = network.open_node_config(host_id)
        authorized_page.click(f"#config_host_job_edit_{job_ids[0]}")
        expect(
            authorized_page.locator(
                Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
            )
        ).to_have_value(job_values[job_ids[0]])

        ping_field = authorized_page.locator(
            Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
        )
        ping_field.fill("10.20.30.100")
        config_host.submit()

        # Edit second job
        config_host = network.open_node_config(host_id)
        authorized_page.click(f"#config_host_job_edit_{job_ids[1]}")
        expect(
            authorized_page.locator(
                Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
            )
        ).to_have_value(job_values[job_ids[1]])

        ping_field = authorized_page.locator(
            Location.Network.ConfigPanel.Host.Job.PING_FIELD.selector
        )
        ping_field.fill("10.20.30.101")
        config_host.submit()
