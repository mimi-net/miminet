...
class TestDeviceConnecting:
    ...
    def test_edges_existence(
        self, authorized_page: Page, network: MiminetTestNetwork, edge_id: int
    ):
        edge = network.edges[edge_id]
        network.open_edge_config(edge)

        # (label updates dynamically)
        from_label = authorized_page.evaluate("$('#edge_source').val()")
        to_label = authorized_page.evaluate("$('#edge_target').val()")
