...
def vk_callback():
    ...
    response = requests.get(
        "https://oauth.vk.com/access_token?client_id=51544060&client_secret=5G1LOaa0ty0zFDmH5cPw&redirect_uri=https://miminet.ru/auth/vk_callback&code="
        + user_code
    )
...
# GitHub Actions
config_data = json.loads(os.environ["CLIENT_YANDEX"])
...
def yandex_login():
    ...
    yandex_session = OAuth2Session(client_id, redirect_uri=redirect_uri, scope=scope)
    ...
    try:
        ...
        if user is None:
            ...
            if avatar_uri:
                ...
                avatar_uri += ".jpg"
                avatar_url = (
                    f"https://avatars.yandex.net/get-yapic/{avatar_uri}/islands-200"
                )
        ...
        flash(
            f"Unexpected error occurred during callback processing: {e}",
            category="error",
        )
        return redirect(url_for("login_index"))
