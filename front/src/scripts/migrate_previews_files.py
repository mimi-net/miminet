#!/usr/bin/env python3
"""Migrate preview images to hash-based nested directories.

Supports production-friendly phased migration:
1) move files from old flat directory to new bucket layout
2) update DB preview_uri from "file.png" to "a/b/file.png"
3) rerun safely (idempotent)

Examples:
    # Preview actions (no changes)
    python front/src/scripts/migrate_previews_files.py --dry-run

    # Migrate files only from legacy directory
    python front/src/scripts/migrate_previews_files.py \
      --legacy-root /mnt/old-previews --only-files

    # Update database references only
    python front/src/scripts/migrate_previews_files.py --only-db
"""
...
def ensure_db_models_loaded():
    ...
    if Network is not None and db is not None:
        return

    from miminet_model import Network as loaded_network, db as loaded_db  # noqa: E402
...
def is_bucketized_preview_uri(preview_uri: str) -> bool:
    parts = preview_uri.split("/")
    if len(parts) != 3:
        return False
    ...
    return preview_uri == build_target_rel(file_name)


def is_legacy_flat_preview_uri(preview_uri: str) -> bool:
    return Path(preview_uri).name == preview_uri and preview_uri not in {"", ".", ".."}
...
def migrate_files(source_root: Path, preview_root: Path, dry_run: bool) -> tuple[int, int, int]:
    ...
    for entry in iter_flat_files(source_root):
        ...
        if file_name in SYSTEM_PREVIEWS:
            skipped += 1
            continue

        target_rel = build_target_rel(file_name)
        target_abs = preview_root / target_rel
...
def migrate_db(batch_size: int, dry_run: bool) -> tuple[int, int]:
    ...
    query = Network.query.with_entities(Network.id, Network.preview_uri)

    try:
        for network_id, preview_uri in query.yield_per(batch_size):
            if not preview_uri or preview_uri in SYSTEM_PREVIEWS:
                skipped += 1
                continue

            if is_bucketized_preview_uri(preview_uri):
                skipped += 1
                continue
            ...
            new_preview_uri = build_target_rel(preview_uri)
            print(f"[DB] network_id={network_id}: {preview_uri} -> {new_preview_uri}")
            updated += 1

            if dry_run:
                continue

            Network.query.filter(Network.id == network_id).update(
                {"preview_uri": new_preview_uri}, synchronize_session=False
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Target preview root where files should end up",
    )
    parser.add_argument(
        "--legacy-root",
        type=Path,
        default=None,
        help="Optional source dir with old flat files. Defaults to --preview-root",
    )
...
def main() -> int:
    ...
    legacy_root: Path = args.legacy_root or preview_root

    if not preview_root.exists():
        print(f"[ERROR] Preview root does not exist: {preview_root}")
        return 1
