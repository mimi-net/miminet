...
@pytest.fixture
def create_hash():
    BOT_TOKEN = json.loads(os.environ.get("BOT_TOKEN"))
...
def test_tg_callback_handles_user_info_and_creates_user_in_db(app, mocker):
    ...
    """
    with app.test_request_context():
        user_json = {"id": "test_id", "first_name": "test_name", "username": "test_username"}
...
def test_tg_callback_handles_exception(app, mocker):
    ...
    with app.test_request_context():
        ...
        with pytest.raises(Exception):
            response = tg_callback()
            mock_logger.assert_called_with('Error while processing Telegram callback: %s', 'Test exception')
