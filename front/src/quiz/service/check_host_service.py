...
def check_no_echo_request(answer, source_device, target_device):
    ...
    for packet in packets:
        ...
        if ("ICMP echo-request" in packet_type and 
            source == source_device and 
            target == target_device):
            request_reached_target = True
            break
            
    if request_reached_target:
        hints.append(f"Обнаружен ping от {source_device} к {target_device}, хотя он не должен был проходить")
        return False, hints
    else:
        return True, []
