    ...
    for node in nodes:
        ...
        for interface in interfaces:
            ...
            if ip and not is_private_ip(ip):
                return False, f"Ip-адрес {ip} не является приватным, хотя по условию все адреса в сети должны быть приватными."
        ...
        for interface in node.get("interface", []):
            ...
            if isinstance(vlan_id, int):
                if vlan_id < vlan_id_threshold:
                    return False, f"В сети есть VLAN с ID {vlan_id}. Но VLAN ID должны быть больше {vlan_id_threshold}."
            
            elif isinstance(vlan_id, list):
                for vlan in vlan_id:
                    if vlan < vlan_id_threshold:
                        return False, f"В сети есть VLAN с ID {vlan}. Но VLAN ID должны быть больше {vlan_id_threshold}."
...
def check_network_configuration(requirement, answer):
    ...
    if ip_private: 
        if check_network_ip_private(answer):
